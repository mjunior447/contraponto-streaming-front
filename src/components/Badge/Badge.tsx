interface BadgeProps {
    text: string;
    variant?: 'tech' | 'disabled' | 'tooltip' | 'brand' | 'featured' | 'about';
    className?: string;
}
...
export function Badge({ text, variant = 'tech', className = '' }: BadgeProps) {
    const styles = {
        ...
        featured: 'text-[10px] bg-brand-accent/20 border border-brand-accent/40 text-brand-accent font-black px-2.5 py-1 rounded tracking-cinematic uppercase',
        about: 'text-xs bg-brand-accent/10 border border-brand-accent/30 text-brand-accent font-bold px-3 py-1 rounded-full uppercase tracking-wider'
    };
